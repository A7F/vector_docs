[Open topic with navigation](../../../../../../CANoeDEFamily.htm#Topics/CAPLFunctions/ISO11783/ISOInteractionLayerFS/Functions/CAPLfunctionIso11783FSILFreezeTp.md)

**CAPL Functions** » **ISO11783** » **File Server Interaction Layer (FS IL)** » **FSIL_FreezeTp**
...
```plaintext
long FSIL_FreezeTp(dword protocol, dword sa, dword da); //Form 1
long FSIL_FreezeTp(dbNode node, dword protocol, dword sa, dword da); //Form 2
```
...
To freeze a connection immediately you need to specify a delay for the transport protocol messages of 1 ms or greater, since the interaction layer will react on a message as soon as possible, and this may be before the CAPL function is handled. To setup delays for transport protocol messages, refer to the CAPL function [FSIL_SetNodeProperty](CAPLfunctionIso11783FSILSetNodeProperty.md).
...
{
  // Connection Node1 (originator) --> Nodes2 (receiver)
  FSIL_UnfreezeTp(kTpRtsCts, 0x1, 0x2);
}
```
