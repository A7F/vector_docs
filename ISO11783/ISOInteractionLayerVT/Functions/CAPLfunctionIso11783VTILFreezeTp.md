[Open topic with navigation](../../../../../../CANoeDEFamily.htm#Topics/CAPLFunctions/ISO11783/ISOInteractionLayerVT/Functions/CAPLfunctionIso11783VTILFreezeTp.md)

[CAPL Functions](../../../CAPLfunctions.md) » [ISO11783](../../CAPLfunctionsISO11783Overview.md) » [Virtual Terminal Interaction Layer (VT IL)](../CAPLfunctionsISOILVTOverview.md) » VTIL_FreezeTp
...
```plaintext
long VTIL_FreezeTp(dword protocol, dword sa, dword da); // Form 1
long VTIL_FreezeTp(dbNode node, dword protocol, dword sa, dword da); // Form 2
```
...
To freeze a connection immediately you need to specify a delay for the transport protocol messages of 1 ms or greater, since the interaction layer will react on a message as soon as possible, and this may be before the CAPL function is handled. To setup delays for transport protocol messages, refer to the CAPL function [VTIL_SetNodeProperty](CAPLfunctionIso11783VTILSetNodeProperty.md).
...
on start
{
  ...
  VTIL_SetNodeProperty("TPDTLatency", 2);
  VTIL_SetNodeProperty("CTSLatency", 2);
  VTIL_SetNodeProperty("EoMALatency", 2);
}

on key 'f'
{
  // Connection Node1 (originator) --> Nodes2 (receiver)
  VTIL_FreezeTp(kTpRtsCts, 0x1, 0x2);
}

on key 'u'
{
  // Connection Node1 (originator) --> Nodes2 (receiver)
  VTIL_UnfreezeTp(kTpRtsCts, 0x1, 0x2);
}
```
