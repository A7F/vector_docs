...
The packet is sent immediately. All messages that were prepared for sending beforehand with [AREthPostMessage](CAPLfunctionAREthPostMessage.md) are inserted in the packet to be sent and sent with the message, provided that the buffered messages have the same source and the same destination.
