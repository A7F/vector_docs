[Open topic with navigation](../../../../../CANoeDEFamily.htm#Topics/CAPLFunctions/IP/Functions/CAPLfunctionEthGetRxPacketRate.md)

**CAPL Functions** » **Ethernet** » **Function Overview** » **ethGetRxPacketRate**
...
```plaintext
qword ethGetRxPacketRate(ethernetPort port, ValueSelector valueSelector);
```

## Description

Returns the RX packet rate of the specified port. The parameter **valueSelector** controls whether the minimum value, the maximum value, the average value or the current value has to be returned.
...
RX packet rate in [Packets/sec], **0** if no valid value currently exists.
